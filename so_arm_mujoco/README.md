...
`so_arm_control`'s `control.launch.py` loads this package's `config/mujoco_ros2_control_plugins.yaml` into `mujoco_ros2_control`'s node, adding a simulated `/emergency_stop` (`mujoco_ros2_plugins/EmergencyStopPlugin`) and, for SO101 with `wrist_camera_urdf:=true`, the wrist camera (`mujoco_ros2_control_plugins/CameraPlugin`, `config/mujoco_ros2_control_plugins_so101_camera.yaml`) on the same `/image_raw`/`/camera_info` topics the real driver uses. The simulated e-stop **holds the last commanded position** rather than releasing torque (matching `mujoco_ros2_plugins`' behavior for lekiwi/pantilt's position-controlled joints, not this arm's actual gravity-drop on power cut) — it exercises the ROS graph, not the physical failure mode; see the [repository README](../README.md)'s safety section. This mode is for integration testing, not for validating hardware behavior — the underlying dynamics are still uncalibrated (see [Limitations](#limitations)).
