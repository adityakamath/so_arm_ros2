...
`so_arm_control`'s `control.launch.py` loads this package's `config/mujoco_ros2_control_plugins.yaml` into `mujoco_ros2_control`'s node, adding a simulated `/emergency_stop` (`mujoco_ros2_plugins/EmergencyStopPlugin`, with `release_on_stop: true`) and, for SO101 with `wrist_camera_urdf:=true`, the wrist camera (`mujoco_ros2_control_plugins/CameraPlugin`, `config/mujoco_ros2_control_plugins_so101_camera.yaml`) on the same `/image_raw`/`/camera_info` topics the real driver uses. The simulated e-stop **releases torque and joint damping**, so the arm falls under gravity (plus its own frictionloss) — matching the standalone tools' `keyboard.py` `EStop` and the real gravity-drop on power cut. The underlying dynamics are still uncalibrated (see [Limitations](#limitations)), so treat this as integration testing of the ROS graph and failure-mode behavior, not a precise physics match.
